...
class ExactInference(object):
	...
	def observe(self, agentX, agentY, observedDist):
		...
		rows_num = self.belief.getNumRows()
		cols_num = self.belief.getNumCols()
		...
		for row in range(rows_num):
			...
			for col in range(cols_num):
				...
				condition = util.pdf(observedDist, Const.SONAR_STD, 
									 math.hypot(agentX - colX, agentY - rowY))
				new_prob = prob * condition
				self.belief.setProb(row, col, new_prob)
	...
	def elapseTime(self):
		...
		for row in range(rows_num):
			for col in range(cols_num):
				seen[(row, col)] = self.belief.getProb(row, col)
				self.belief.setProb(row, col, 0)

		for item in self.transProb:
			old, new = item
			delta_prob = self.transProb[item] * seen[old]
			self.belief.addProb(new[0], new[1], delta_prob)
...
class ParticleFilter(object):
	...
	def __init__(self, numRows, numCols):
		...
		# self.transProbDict[oldTile][newTile] = probability of transitioning
		...
		for (oldTile, newTile) in self.transProb:
			...
			self.transProbDict[oldTile][
				newTile] = self.transProb[(oldTile, newTile)]
	...
	def observe(self, agentX, agentY, observedDist):
		...
		for item in self.particles:
			prob = self.particles[item]
			X = util.colToX(item[1])
			Y = util.rowToY(item[0])
			mean = math.hypot(agentX - X, agentY - Y)
			new_prob = prob * util.pdf(mean, Const.SONAR_STD, observedDist)
			new_particles[item] = new_prob

		self.particles = collections.defaultdict(int)
		for i in range(self.NUM_PARTICLES):
			new_tile = util.weightedRandomChoice(new_particles)
			self.particles[new_tile] += 1
		# END_YOUR_CODE
		self.updateBelief()
	...
	def elapseTime(self):
		...
		for item in self.particles:
			num = self.particles[item]
			for i in range(num):
				new_tile = util.weightedRandomChoice(self.transProbDict[item])
